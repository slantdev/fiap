      ...
      <div class="px-14 relative">
        <div id="case-studies-filter" class="swiper mt-10 px-2 pt-2 pb-4">
          <div class="swiper-wrapper filter-case-study-buttons">
            ...
            if (!empty($taxonomies)) :
              $output = '<div class="swiper-slide w-auto"><button type="button" class="filter-case-study filter-active inline-block rounded-full px-10 py-2.5 bg-white border border-neutral-100 shadow-md hover:shadow-lg transition-all duration-200" data-id="all">All</button></div>';
              foreach ($taxonomies as $category) {
                $output .= '<div class="swiper-slide w-auto"><button type="button" class="filter-case-study inline-block rounded-full px-10 py-2.5 bg-white border border-neutral-100 shadow-md hover:shadow-lg transition-all duration-200" data-id="' . esc_attr($category->term_id) . '">' . esc_attr($category->name) . '</button></div>';
              }
              echo $output;
            endif;
            ?>
        ...
        <button type="button" class="casestudy-filter--button-prev absolute top-[13px] left-0 w-10 h-10 flex items-center justify-center bg-white rounded-full shadow-md hover:bg-fiap-teal text-gray-500 hover:text-white transition-all duration-200">
        ...
        <button type="button" class="casestudy-filter--button-next absolute top-[13px] right-0 w-10 h-10 flex items-center justify-center bg-white rounded-full shadow-md hover:bg-fiap-teal text-gray-500 hover:text-white transition-all duration-200">
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
